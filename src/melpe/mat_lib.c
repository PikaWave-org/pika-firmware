...
#include <stdlib.h>
#include "pika_no_stdio.h"	/* pika: not upstream, see PROVENANCE.md */
...
/* pika: the codec's scratch vectors came from malloc, which this firmware
 * has no heap for. Every v_get is released by a v_free before its function
 * returns, so the allocator is a bump pointer over a fixed arena: freeing a
 * block also releases everything handed out after it. That covers the one
 * site that frees out of order (lsp_to_lpc's f0/f1), which frees both
 * without using either in between.
 *
 * The peak is structural rather than signal dependent - the block sizes come
 * from LPC_ORD and the frame length, not from the audio. Measured at 161
 * int32_t words for encode plus decode at 1200 bps, the same for silence,
 * noise, a sweep, a buzz and a full scale square wave, so 256 leaves a wide
 * margin in 1KB. Raise it if the assert in melpe_arena_get ever fires. */
#define MELPE_ARENA_WORDS	256

static int32_t melpe_arena[MELPE_ARENA_WORDS];
static int32_t *melpe_arena_top = melpe_arena;

static void *melpe_arena_get(int32_t size)
{
	int32_t words = (size + (int32_t) sizeof(int32_t) - 1) /
	    (int32_t) sizeof(int32_t);
	int32_t *ptr = melpe_arena_top;

	assert(ptr + words <= melpe_arena + MELPE_ARENA_WORDS);
	melpe_arena_top = ptr + words;
	return (ptr);
}

int16_t *v_get(int16_t n)
{
	return (int16_t *) melpe_arena_get(sizeof(int16_t) * n);
}

int32_t *L_v_get(int16_t n)
{
	return (int32_t *) melpe_arena_get(sizeof(int32_t) * n);
}

void v_free(void *v)
{
	if (v && (int32_t *) v < melpe_arena_top)
		melpe_arena_top = (int32_t *) v;
}
