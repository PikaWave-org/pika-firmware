...
/* pika: the ETSI basic ops validate every 40 bit accumulator on entry, and
 * these helpers are always_inline, so the check lands at every call site -
 * an fprintf, an exit and a libm floor() each time. The floor() term is also
 * meaningless here: acc is an int64_t, so it can only ever differ from its
 * own floor by way of the double conversion above 2^53. Firmware gets the
 * arithmetic without the check; define MELPE_BASIC_OP_CHECKS on a host build
 * to get it back. */
#ifdef MELPE_BASIC_OP_CHECKS
#include <stdio.h>
#include <stdlib.h>
#define MELPE_CHECK_40(acc)						\
	do {								\
		if (((acc) > MAX_40) || ((acc) < MIN_40)) {		\
			fprintf(stderr,					\
				"BASIC_OP: Error in 40 bits format.\n");	\
			exit(0);					\
		}							\
	} while (0)
#else
#define MELPE_CHECK_40(acc)	((void) 0)
#endif
